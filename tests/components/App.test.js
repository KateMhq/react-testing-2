...
const Today = {
  ...
  test('update the state with results', () => {
    ...
    return instance.submitSearch("Today")
      .then(()=> {

        expect(fetch).toHaveBeenCalledWith(`http://www.omdbapi.com/?s=Today&apikey=2cda7206`)
  ...
  test('research results are undefined', () => {
    ...
    return instance.submitSearch('djlkdjfls')
      .then(() => {
        const moviesState = wrapper.state('movies')
